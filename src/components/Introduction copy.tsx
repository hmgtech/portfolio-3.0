...
const Introduction: React.FC = () => {
  ...
  const [scrollRocketBoost, setScrollRocketBoost] = useState(false);
  ...
  const handleBeginJourneyClick = () => {
    ...
    setShutterOpen(true);
    setScrollRocketBoost(true); // Boost flame for scroll rocket
    ...
    // Close the shutter and reset flame boost after a short delay (1 second after scroll start)
    setTimeout(() => {
      setShutterOpen(false);
      setScrollRocketBoost(false);
    }, 1600);
  };
  ...
  return (
    <section id="intro" className="min-h-screen flex items-center justify-center relative py-20 overflow-hidden">
      ...
      <div
        className={`fixed left-0 bottom-0 w-full z-[60] pointer-events-none`}
        style={{
          height: shutterOpen ? '100vh' : '0',
          background: 'black',
          transition: 'height 0.5s cubic-bezier(0.4,0,0.2,1)',
          overflow: 'visible',
        }}
      >
        {/* Rocket dragging the shutter */}
        {shutterOpen && (
          <div
            className="absolute left-1/2"
            style={{
              bottom: 'calc(100% - 60px)', // position rocket just above shutter
          ...
          >
            <div className="flex flex-col items-center">
              {/* Rocket */}
              <div
                className="relative w-12 h-20 sm:w-16 sm:h-28 flex items-center justify-center"
                style={{ zIndex: 62 }}
              >
                {/* Rocket Body */}
                ...
                <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-4 h-8 sm:w-6 sm:h-12">
                  <div
                    className="w-4 h-8 sm:w-6 sm:h-12 bg-gradient-to-b from-yellow-400 via-orange-500 to-red-600 rounded-full blur-sm opacity-100 scale-y-150 animate-flame-boost"
                    style={{
                      boxShadow: '0 0 40px 20px orange, 0 0 80px 40px yellow',
                      transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)',
                    }}
                  ></div>
                  <div
                    className="absolute top-2 left-1 w-2 h-5 sm:left-1.5 sm:w-3 sm:h-6 bg-gradient-to-b from-white to-yellow-400 rounded-full opacity-100 scale-y-125 animate-flame-boost-delay"
                    style={{
                      boxShadow: '0 0 20px 10px yellow',
                      transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)',
                    }}
                  ></div>
                  <div className="absolute bottom-[-8px] left-1/2 transform -translate-x-1/2 w-8 h-8 bg-gradient-to-b from-yellow-300 via-orange-400 to-transparent rounded-full blur-xl opacity-80 animate-flame-burst"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Scroll to Explore button (hidden when shutter is open) */}
      {!shutterOpen && (
        <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 z-10">
          ...
          >
            {/* Rocket */}
            <div
              className={`relative w-12 h-20 sm:w-16 sm:h-28 flex items-center justify-center animate-rocket-bounce transition-transform duration-500`}
              style={{
                zIndex: 51,
            ...
            >
              ...
              <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-4 h-8 sm:w-6 sm:h-12">
                <div
                  className={`w-4 h-8 sm:w-6 sm:h-12 bg-gradient-to-b from-yellow-400 via-orange-500 to-red-600 rounded-full blur-sm opacity-80 ${
                    scrollRocketBoost ? 'scale-y-150 opacity-100 animate-flame-boost' : 'animate-flame-flicker'
                  }`}
                  style={{
                    boxShadow: scrollRocketBoost
                      ? '0 0 40px 20px orange, 0 0 80px 40px yellow'
                      : '0 0 12px 6px orange',
                    transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)',
                  }}
                ></div>
                <div
                  className={`absolute top-2 left-1 w-2 h-5 sm:left-1.5 sm:w-3 sm:h-6 bg-gradient-to-b from-white to-yellow-400 rounded-full opacity-70 ${
                    scrollRocketBoost ? 'scale-y-125 opacity-100 animate-flame-boost-delay' : 'animate-flame-flicker-delay'
                  }`}
                  style={{
                    boxShadow: scrollRocketBoost
                ...
                ></div>
                {/* Extra flame burst when boosting */}
                {scrollRocketBoost && (
                  <div className="absolute bottom-[-8px] left-1/2 transform -translate-x-1/2 w-8 h-8 bg-gradient-to-b from-yellow-300 via-orange-400 to-transparent rounded-full blur-xl opacity-80 animate-flame-burst"></div>
                )}
              </div>
